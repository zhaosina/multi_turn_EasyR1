...
def check_hit_with_converted_bbox(pred_rel, bbox_gt_rel):
...
def main():
    ...
    for task in tasks:
        ...
        for idx, item in tqdm(list(enumerate(data)), desc=f"{task}"):
            ...
            bbox_xywh = item["bbox"]  # ScreenSpot-v2 格式: [x, y, width, height]
            bbox_xyxy = convert_bbox_format(bbox_xywh)  # 转换为 [x1, y1, x2, y2]
            ...
            logging.info(f"[{task}][{idx}] 原始bbox(xywh): {bbox_xywh}, "
                        f"转换后bbox(xyxy): {bbox_xyxy}, "
                        f"相对坐标: {bbox_gt_rel}")
            ...
            for iteration in range(args.num_iterations):
                ...
                try:
                    ...
                    hit = check_hit_with_converted_bbox(pred_rel, bbox_gt_rel)
